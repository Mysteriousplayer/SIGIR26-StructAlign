...
class CLIPVisionViPEmbeddings(nn.Module):
    def __init__(self, config: CLIPVisionConfig, additional_vision_config=None):
        ...
        # self.added_cls = nn.Parameter(torch.zeros(self.add_cls_num, self.embed_dim))
...
class CLIPEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if isinstance(hidden_states, tuple):
            ...
            nan_mask = torch.isnan(cls_token)
            samples_with_nan = nan_mask.any(dim=1)
            if samples_with_nan.any():
                print('nan in cls token')
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        last_hidden_state = encoder_outputs[0]
        pooled_output = last_hidden_state[:, 0, :]  # take features from the CLS token
