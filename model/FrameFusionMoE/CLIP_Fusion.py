...
from model.FrameFusionMoE.adapter import FrameCrossAttention, LoRAAdapter
...
class CLIPTextTransformer(nn.Module):
    ...
    def forward(
        ...
        # Pad to max_middle_len
        # middle_token_embeds_padded = torch.zeros(batch_size, max_middle_len, hidden_dim, device=last_hidden_state.device, dtype=last_hidden_state.dtype)
        # middle_token_ids_padded = torch.zeros(batch_size, max_middle_len, device=last_hidden_state.device, dtype=input_ids.dtype)
        # for b in range(batch_size):
        #     l = middle_token_embeds[b].shape[0]
        #     if l > 0:
        #         middle_token_embeds_padded[b, :l, :] = middle_token_embeds[b]
        #         middle_token_ids_padded[b, :l] = middle_token_ids[b]
        # -----------------------------------------------------------------
...
class CLIPModel(CLIPPreTrainedModel):
        ...
        if prototypes is None:
            ...
            for prototype_id in range(1, prototypes+1):
                ...
                for emb in middle_embeds:  # middle_embeds 是 list
                    if emb.shape[0] > 0:  # 有 token
                        projected_middle_embeds.append(self.text_projection(emb))
                        # print("c:",self.text_projection(emb).shape)
                    else:
                        # 如果没有 token，可以返回空 tensor
                        projected_middle_embeds.append(emb.new_zeros((0, emb.shape[1])))
                        # print("cc:")
                ...
                if if_norm:
                    text_features = [F.normalize(t, dim=-1, eps=1e-8) if t.shape[0]>0 else t
                                       for t in projected_middle_embeds]
                    # text_features = text_features / text_features.norm(dim=-1, keepdim=True)
                # print('ccc:',len(text_features))
                # print('cccc:',text_features[0].shape)
                # Append to list
                text_features_array.append(text_features)
            # print('d:',len(text_features_array))
            # Stack all text features into a single tensor with shape [p, n, dim]
            # where p is prototype_id, n is number of tokens, dim is feature dimension
            # text_proto_features = torch.stack(text_features_array, dim=0)
    ...
    def forward(
        ...
        # middle_embeds = self.text_projection(middle_embeds)
        middle_ids = text_outputs[3]
        # sims, mats = tokenwise_word_frame_similarity_from_lists(frame_embeds, middle_embeds, return_matrices=True)
        sims = batch_word_frame_similarity_variable_text(frame_embeds, projected_middle_embeds)
        # print('sims_wf:',sims * self.logit_scale.exp())
        # sims_ = batch_sentence_frame_similarity(frame_embeds, projected_middle_embeds)
        # print('sims_sf:',sims_ * self.logit_scale.exp())
...
def batch_word_frame_similarity_variable_text(
    ...
    # Precompute (for efficiency) video -> frames transpose [B, D, N]
    # frames_t = frames.transpose(1, 2)  # [B, D, N]
    ...
    for i in range(B):  # each video
        ...
        for j in range(B):  # each text
            ...
            # Eq.7: (mean of max over opposite directions)
